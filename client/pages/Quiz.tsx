...
const Quiz = ({ currentStep = 0, t, quizData = {}, updateQuizData = () => {} }: Props) => {
  ...
  // Safe stringify for debug rendering
  let debugStr = "";
  try {
  ...
  // If we're not on the quiz step, render nothing (but still show debug overlay)
  ...
  return (
    <div>
      {/* Visible debug overlay so we can inspect runtime props without using DevTools */}
        ...
        <div className="p-6 text-white min-h-[200px]">
          ...
          {platforms.length === 0 ? (
            <div className="text-sm text-gray-300">
              No platforms available yet. The debug panel (top-right) shows the props received.
            </div>
          ) : (
